...
class RandomCharacterDeletion(keras.layers.Layer):
    """Augments input by randomly deleting words.

    The layer works by splitting the words using `tf.strings.split` into words 
    and then uses `tf.strings.unicode_split` to split into characters

    Args:
        probability: probability of a character in a word to be chosen for deletion
        max_deletions: The maximum number of characters to delete per word
    ...
    >>> augmenter = keras_nlp.layers.RandomCharacterDeletion(
    ...     probability = 0.7,
    ...     max_deletions = 2,
    ... )
    >>> augmenter(["I like to fly kites, do you?",
    ...     "Can we go fly some kites later?"])
    <tf.Tensor: shape=(2,), dtype=string, numpy=array([b'I li to fly kie, do o?',
    b'Cn we f ite ater?'], dtype=object)>
    """
    ...
    def call(self, inputs):
        ...
        ragged_words = tf.strings.unicode_split(ragged_words, 'UTF-8')
